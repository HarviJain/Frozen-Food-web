...
# ──────────────────────────────────────────────
#  EMAIL CONFIGURATION (DIRECTLY HARDCODED)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
#  EMAIL SENDER FUNCTION (UPDATED WITH HARDCODED CREDENTIALS)
# ──────────────────────────────────────────────

def send_enquiry_email(enquiry):
    """Send enquiry details to food@abhyuday.in"""
    try:
        # Email content
        subject = f"📩 New B2B Enquiry from {enquiry.name} - {enquiry.company}"
        
        # HTML Email Body (Professional formatting)
        html_body = f"""
        <!DOCTYPE html>
        <html>
        <head>
            <style>
                ...
                .header {{ background: #C0541A; color: white; padding: 15px; text-align: center; }}
                .content {{ padding: 20px; }}
                .field {{ margin-bottom: 15px; }}
                .label {{ font-weight: bold; color: #C0541A; }}
                .message {{ background: white; padding: 15px; border-left: 4px solid #C0541A; margin-top: 10px; }}
        ...
        <body>
            <div class="container">
                <div class="header">
                    ...
                    <p>Abhyuday Bharat Food Cluster</p>
                </div>
                <div class="content">
                    <div class="field">
                    ...
                    <div class="field">
                        <span class="label">📞 Phone:</span> <a href="tel:{enquiry.phone}">{enquiry.phone}</a>
                    </div>
                    <div class="field">
                        <span class="label">📧 Email:</span> <a href="mailto:{enquiry.email or 'N/A'}">{enquiry.email or 'N/A'}</a>
                    </div>
                    <div class="field">
                        <span class="label">💼 Business Type:</span> {enquiry.business_type or 'Not specified'}
                    </div>
                    <div class="message">
                        <span class="label">📝 Message:</span>
                        <p>{enquiry.message or 'No message provided'}</p>
                    </div>
                    <div class="field">
                        <span class="label">📅 Date:</span> {datetime.now().strftime('%B %d, %Y at %I:%M %p')}
                    </div>
                </div>
                <div class="footer">
                    <p>This enquiry was submitted from the Abhyuday Foods website contact form.</p>
                    <p>You can reply directly to this email to contact the customer.</p>
                    <hr>
                    <small>Abhyuday Bharat Food Cluster | Premium Frozen Foods</small>
                </div>
            </div>
        </body>
        </html>
        ...
        # Set Reply-To to customer's email if available
        if enquiry.email:
            msg['Reply-To'] = enquiry.email
        ...
        print(f"✅ Enquiry email sent successfully to {EMAIL_RECIPIENT}")
        print(f"   From: {EMAIL_SENDER}")
        print(f"   Customer: {enquiry.name} - {enquiry.company}")
        return True
...
# ──────────────────────────────────────────────
#  ENQUIRY SUBMISSION (UPDATED WITH EMAIL)
# ──────────────────────────────────────────────
...
def submit_enquiry():
    ...
    response_data = enq.to_dict()
    response_data['email_sent'] = email_sent

    return ok(response_data, 'Enquiry submitted successfully' + (' Email notification sent.' if email_sent else ''))
...
def seed_database():
    ...
    if not Product.query.first():
        products = [
            ...
            ('millet','Malt','Ragi Malt (Sweet)','1 KG','src/products/Ragi Malt (Sweet).png','Pre-sweetened with jaggery.',['Millet'],19),
            ('millet','Chilla','Sprouted Moong Chilla','1 KG','src/products/Sprouted Moong Chilla.png','High protein bioavailability.',['Millet'],20),
            ('millet','Chilla','Sprouted Moong + Foxtail Chilla','1 KG','src/products/foxtail chilla.png','Low-GI balanced protein chilla.',['Millet'],21),
            ('millet','Chilla','Sprouted Moong + Jowar Chilla','1 KG','src/products/jowar chilla.png','Gluten-free high-fibre chilla.',['Millet'],22),
            ('millet','Soup','Bajra Soup','1 KG','src/products/Bajra Soup.png','Iron-rich bajra soup.',['Millet'],23),
            ('millet','Pasta Sauce','Oats White Pasta Sauce (GF)','1 KG','src/products/White-Gravy.jpg','Gluten-free pasta sauce.',['Millet'],24),
            # NON-MILLET
